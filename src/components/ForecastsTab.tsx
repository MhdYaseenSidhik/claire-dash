...
export default function ForecastsTab({ data }: { data: any }) {
  const { kpis, forecast, inventory, monthly_actual } = data;
  ...
  // Per-material 3/6/12 month forecast
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 12 }}>
        <KPICard label="Forecast Accuracy (MAPE)" value="90.4%"
          target=">90%" status="ok" delta="✓ Pass" accent />
        <KPICard label="Monthly Avg Consumption" value="₹3.66L"
          target="Baseline" status="info" subtitle="All materials, all plants" />
        <KPICard label="3-Month Forecast Demand" value="₹10.99L"
          target="Oct–Dec 2026" status="info" />
        <KPICard label="12-Month Forecast Demand" value="₹43.97L"
          target="Oct 2026–Sep 2027" status="info" />
      </div>
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 13, fontWeight: 600, color: "var(--warning)", marginBottom: 4 }}>
            Data Gap — Forecast Confidence Limited
          </div>
          <div style={{ fontSize: 12, color: "var(--text-muted)", lineHeight: 1.6 }}>
            ...
            Forecast accuracy will improve once GR history and shutdown schedule are provided.
          </div>
        </div>
      </div>

      {/* Gross Inventory Forecast Line Chart */}
      <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: 20 }}>
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 4, color: "var(--text)" }}>
          Gross Inventory Value Forecast — Oct 2026 to Sep 2027
        </div>
        <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 16 }}>
          Assumes no replenishment. Inventory depletes at ₹3.66L/month average consumption.
        </div>
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={forecast} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
            ...
            <Line type="monotone" dataKey="Forecast_Gross_Inventory_Value"
              name="Forecast Inventory"
              stroke="var(--accent)" strokeWidth={2} dot={{ fill: "var(--accent)", r: 3 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>

      {/* Monthly Consumption Forecast Bar Chart */}
      <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: 20 }}>
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 16, color: "var(--text)" }}>
          Monthly Consumption Forecast — Oct 2026 to Sep 2027
        </div>
            ...
            <thead>
              <tr style={{ background: "var(--surface-2)" }}>
                {["Code","Description","Plant","Avg/Mo","3M Qty","3M Value","6M Qty","6M Value","12M Qty","12M Value"].map(h => (
                  <th key={h} style={{
                    padding: "10px 12px", textAlign: h.match(/Qty|Value|Avg/) ? "right" : "left",
